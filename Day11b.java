...
public class Day11b {
    public static void main(String [] args) {
        ...
        long answer;
        int monkeyCount = 0;
        long[][] monkeyItems = new long [8][30];
        ...
        try {
            ...
            while (br.readLine() != null) {
                ...
                alltests = alltests * test [monkeyCount];
                thisLine=br.readLine();
                e = thisLine.split(" ");
            ...
            for(int round = 1; round < 10001; round ++){
                for(int monkey = 0; monkey < monkeyCount; monkey ++){
                    for(int item = 0; item < itemCount[monkey]; item ++){
                        long firstval = monkeyItems[monkey][item];
                        long secondval;
                       ...
                       if(firstval % test[monkey] == 0){
                            monkeyItems[target[monkey][0]][itemCount[target[monkey][0]]] = firstval % alltests;
                            itemCount[target[monkey][0]] ++;
                        } else {
                            monkeyItems[target[monkey][1]][itemCount[target[monkey][1]]] = firstval % alltests;
            ...
            for(int x = 0; x < monkeyCount; x ++){
                System.out.println(x + " " + inspections[x]);
            }
            Arrays.sort(inspections);
            answer = (long) inspections[inspections.length - 1] * (long) inspections[inspections.length - 2];
